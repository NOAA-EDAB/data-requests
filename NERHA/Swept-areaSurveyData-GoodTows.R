...
#Albatross and Deleware data 1963-2008, all seasons:
qry <- c(
  ...
  b.svspp, b.catchsex, expcatchwt, expcatchnum, length, expnumlen, s.svvessel
...
saveRDS(survey,file = here::here(out.dir,paste0("NEFSC_1963-2008survey_data_sweptarea",dateOfPull,".rds")))


#Bigelow data, 2009-present, spring and fall: 
#If you include TOW_EVALUATION, it limits data to starting in 2009
qry <- c(
  "select b.cruise6,b.stratum,b.tow,b.station,
  s.est_year year,season, est_month month,est_day day,
  substr(est_time,1,2)||substr(est_time,4,2) time,
  ...
  round(((substr(endlon,1,2) + (substr(endlon,3,7)/60)) * -1), 6) endlon,
  towdur, setdepth, surftemp, bottemp,
  b.svspp, b.catchsex, expcatchwt, expcatchnum, length, expnumlen, s.svvessel, MEAN_SOG_KNOTS, MEAN_DOOR_SPRD_METERS, MEAN_WING_SPRD_METERS, MEAN_HEIGHT_METERS
...
dateOfPull <- format(Sys.time(), "%m-%d-%y")
saveRDS(survey,file = here::here(out.dir,paste0("NEFSC_2009-2023survey_data_sweptarea",dateOfPull,".rds")))
